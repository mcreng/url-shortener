...
app.route('/api/url')
    .get((req, res) => {
      knex('url')
          ...
          .then(r => {
            ...
            console.log(
                `Requested all url/surl data from ${req.session.user_id}.`);
          });
    })
    .post((req, res) => {
    ...
    .delete((req, res) => {
      knex('url')
          ...
          .then(() => {
            console.log(
                `${req.body.surl} is deleted by id ${req.session.user_id}`);
            res.sendStatus(200);
          });
    });

app.route('/api/redirect').post((req, res) => {
  knex('url').select('url').where('surl', req.body.surl).then(r => {
    if (r.length == 0)
      res.sendStatus(404);
    else
      res.status(302).send({url: r[0].url});
...
app.route('/api/auth')
    .post((req, res) => {
      ...
      verifyID(token).catch(console.error).then(ticket => {
...
app.listen(
    port, () => console.log(`Listening on port ${port} ${process.env.MODE}`));
